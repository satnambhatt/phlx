// phalanx-yarn-hook: invoked by the yarn shim. Scans every requested package,
// then execs real yarn with original args. Fail-open on internal errors.
package main
...
// Yarn subcommands that introduce new packages.
var installCmds = map[string]bool{"add": true, "upgrade": true, "up": true}
...
func parseArgs(args []string) []pkgRef {
	...
	for _, a := range args[idx+1:] {
		if strings.HasPrefix(a, "-") || strings.HasPrefix(a, ".") ||
			...
			strings.HasPrefix(a, "git") || strings.HasPrefix(a, "link:") ||
			strings.HasPrefix(a, "workspace:") {
			continue
		}
...
func findRealYarn() string {
	...
	if p, err := exec.LookPath("yarn"); err == nil {
		return p
	}
	return "/usr/local/bin/yarn"
}

func passThrough(realYarn string, args []string) {
	cmd := exec.Command(realYarn, args...)
...
func main() {
	args := os.Args[1:]
	realYarn := findRealYarn()

	packages := parseArgs(args)
	if len(packages) == 0 {
		passThrough(realYarn, args)
		return
	}
	...
	color.New(color.FgCyan, color.Bold).Println("\n  phalanx — scanning yarn packages")
	fmt.Println()
